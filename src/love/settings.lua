...
; "Sudden Death Mode" die if you miss even one note. Health doesn't matter with this enabled
suddenDeath=false

; "Custom Health Gain" decide how much health you gain when you hit a note
; default is 1
customHealthGain=1

; "Custom Health Drain" decide how much health you lose when you miss a note
; default is 2
customHealthDrain=2

; "Extra Lives" add extra lives
; when you run fully out of health, you will lose a life and your health will go back up to 20%
extraLives=0
...
if curOS == "NX" then
	...
	if ini.readKey(settingsIni, "Modifiers", "suddenDeath") == "true" then
	...
	if ini.readKey(settingsIni, "Modifiers", "customHealthGain") == 1 then
		settings.customHealthGain = true
		healthGain = ini.readKey(settingsIni, "Modifiers", "customHealthGain")
	else
		settings.customHealthGain = true
		healthGain = ini.readKey(settingsIni, "Modifiers", "customHealthGain")
	end

	if ini.readKey(settingsIni, "Modifiers", "customHealthDrain") == 1 then
		settings.customHealthDrain = true
		healthDrain = ini.readKey(settingsIni, "Modifiers", "customHealthDrain")
	else
		settings.customHealthDrain = true
		healthDrain = ini.readKey(settingsIni, "Modifiers", "customHealthDrain")
	end

	if ini.readKey(settingsIni, "Modifiers", "customStartingHealth") == 1 then
		startHealth = ini.readKey(settingsIni, "Modifiers", "customStartingHealth")
	else
		startHealth = ini.readKey(settingsIni, "Modifiers", "customStartingHealth")
	end

	if ini.readKey(settingsIni, "Modifiers", "extraLives") == 0 then
		settings.extraLives = false
	else
		extraLives = ini.readKey(settingsIni, "Modifiers", "extraLives")
		settings.extraLives = true
	end
